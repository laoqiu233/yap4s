...
trait ReverseModificationsParser[-C] extends Parser[C] {
  ...
  abstract override def produceRawParseTrees(tokens: Iterable[C]): Seq[SubTree] = {
  ...
  private def reverseModificationSubTree(subTree: SubTree, modificationToReverse: GrammarModification): SubTree = {
    val newTree = subTree match {
      case tree: ModifiedSubTree if tree.appliedModification.modification == modificationToReverse =>
        val (reversedHead, reversedTail) = tree.appliedModification.reverseSubTreeModification(tree.headNode, tree.tailNodes)
        val newTree = tree.appliedModification.parentSubTreeBuilder.buildSubTree(reversedHead, reversedTail)
  ...
  private def reverseModifications(node: MatchResult, modificationToReverse: GrammarModification): MatchResult =
    node match {
      case tree: SubTree => reverseModificationSubTree(tree, modificationToReverse)
      case other => other
    }
}
