...
object RightHandSideDefinition {
  trait PlainRightHandSideDefinition[+T, +C]
    ...
    protected def nextExtract[T1](
        extractor: MatchResultExtractor[T1]
    ): Next[T1, C]
  ...
  case class PlainDefinition0[+Terminal](tokens: Seq[RuleToken[Terminal]])
    ...
    override val resultTransformer: MatchResultExtractor[Nothing] = _ =>
      throw new Exception("Tried to extract from fragment")

    override protected def nextExtract[T1](
        extractor: MatchResultExtractor[T1]
    ): PlainDefinition1[T1, Terminal] =
  ...
  case class PlainDefinition1[A, +Terminal](
      ...
      resultTransformer: MatchResultExtractor[A]
    ...
    override protected def nextExtract[T1](
        extractor: MatchResultExtractor[T1]
    ): PlainDefinition2[A, T1, Terminal] =
  ...
  case class PlainDefinition2[A, B, +Terminal](
      tokens: Seq[RuleToken[Terminal]],
      a: MatchResultExtractor[A],
      b: MatchResultExtractor[B]
    ...
    override protected def nextExtract[T1](
        extractor: MatchResultExtractor[T1]
    ): PlainDefinition2[(A, B), T1, Terminal] =
      PlainDefinition2(tokens, resultTransformer, extractor)
